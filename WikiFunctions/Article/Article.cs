...
public class Article : IProcessArticleEventArgs, IComparable<Article>
{
    ...
    public virtual IAWBTraceListener Trace => mAWBLogListener;
    ...
    /// </summary>
    [XmlIgnore]
    public string NamespacelessName => Tools.RemoveNamespaceString(Name);
    ...
    /// </summary>
    [XmlIgnore]
    public AWBLogListener LogListener => mAWBLogListener; //set { mAWBLogListener = value; } }

    /// <summary>
    /// The name of the article, encoded ready for use in a URL
    /// </summary>
    [XmlIgnore]
    public string URLEncodedName => Tools.WikiEncode(Name);

    /// <summary>
    /// The text of the article. This is deliberately readonly; set using methods
    /// </summary>
    [XmlIgnore]
    public string ArticleText => mArticleText;

    /// <summary>
    /// Article text before this program manipulated it
    /// </summary>
    [XmlIgnore]
    public string OriginalArticleText => mPage == null ? mOriginalArticleText : mPage.Text;

    /// <summary>
    /// Edit summary proposed for article
    /// </summary>
    [XmlIgnore]
    public string EditSummary => summary.ToString();
    ...
    /// </summary>
    [XmlIgnore]
    public bool IsStub => Parsers.IsStub(mArticleText);

    /// <summary>
    /// returns whether the article's title contains any recognised diacritic(s)
    /// </summary>
    [XmlIgnore]
    public bool HasDiacriticsInTitle => (Tools.RemoveDiacritics(Name) != Name);
    ...
    public bool ArticleIsAboutAPerson =>
        Variables.Project == ProjectEnum.wikipedia
    ...
    /// </summary>
    [XmlIgnore]
    public bool IsSIAPage => (Variables.LangCode.Equals("en") || Variables.LangCode.Equals("simple")) && NameSpaceKey == Namespace.Mainspace && WikiRegexes.SIAs.IsMatch(string.Join("", Parsers.GetAllTemplateDetail(mArticleText).ToArray()));

    /// <summary>
    /// Returns whether the article is a disambiguation page has references
    /// </summary>
    [XmlIgnore]
    public bool IsDisambiguationPageWithRefs => IsDisambiguationPage && WikiRegexes.Refs.IsMatch(mArticleText);
    ...
    [XmlIgnore]
    public bool HasRefAfterReflist => Parsers.HasRefAfterReflist(mArticleText);

    /// <summary>
    /// Returns true if the article uses named references ([[WP:REFNAME]]) in short format &lt;ref name=foo /&gt;
    /// </summary>
    [XmlIgnore]
    public bool HasNamedReferences => Parsers.HasNamedReferences(mArticleText);

    /// <summary>
    /// Returns true if the article contains bare references (just the URL link on a line with no description/name)
    /// </summary>
    [XmlIgnore]
    public bool HasBareReferences => Parsers.HasBareReferences(mArticleText);

    /// <summary>
    /// Returns true if the article contains ambiguous dates within a citation template
    /// </summary>
    [XmlIgnore]
    public bool HasAmbiguousCiteTemplateDates => Parsers.AmbiguousCiteTemplateDates(mArticleText);

    /// <summary>
    /// Returns true if the article contains ambiguous dates within a citation template
    /// </summary>
    [XmlIgnore]
    public bool HasSeeAlsoAfterNotesReferencesOrExternalLinks => (NameSpaceKey == Namespace.Article && Parsers.HasSeeAlsoAfterNotesReferencesOrExternalLinks(mArticleText));

    /// <summary>
    /// Returns true if the article should be skipped; check after each call to a worker member. See AWB main.cs.
    /// </summary>
    [XmlIgnore]
    public bool SkipArticle => mAWBLogListener.Skipped;
    ...
    public bool CanDoGeneralFixes =>
        (NameSpaceKey == Namespace.Article
    ...
    /// </summary>
    [XmlIgnore]
    public bool IsMissingReferencesDisplay => Parsers.IsMissingReferencesDisplay(mArticleText);

    /// <summary>
    /// Returns the predominant date locale of the article (may be Undetermined)
    /// </summary>
    [XmlIgnore]
    public Parsers.DateLocale DateLocale => Parsers.DeterminePredominantDateLocale(mArticleText);
    ...
    /// </summary>
    [XmlIgnore]
    public bool OnlyCasingChanged => Tools.CaseInsensitiveStringCompare(OriginalArticleText, mArticleText);
    ...
    public bool OnlyWhiteSpaceAndCasingChanged =>
        Tools.CaseInsensitiveStringCompare(WikiRegexes.WhiteSpace.Replace(OriginalArticleText, ""),
    ...
    /// </summary>
    [XmlIgnore]
    public bool OnlyGeneralFixesChanged => (_generalFixesCausedChange && !FindAndReplaceMadeChanges && !_customModuleMadeChanges && !_categorisationMadeChanges && (ArticleText == _afterGeneralFixesArticleText));
    ...
    public bool FindAndReplaceMadeChanges =>
        (_after == FaRChange.MajorChange || _after == FaRChange.MinorChange
    ...
    /// </summary>
    [XmlIgnore]
    public bool OnlyMinorGeneralFixesChanged => (OnlyGeneralFixesChanged && !_generalFixesSignificantChange);

    /// <summary>
    /// Returns whether the current article text is the same as the original article text
    /// </summary>
    [XmlIgnore]
    public bool NoArticleTextChanged => (String.CompareOrdinal(OriginalArticleText, mArticleText) == 0);
    ...
    string IProcessArticleEventArgs.ArticleTitle => Name;
    ...
    /// </summary>
    [XmlIgnore]
    public bool IsRedirect => Tools.IsRedirect(ArticleText);
